...
async function getSong() {
  try {
    const response = await fetch(
      "https://api.lanyard.rest/v1/users/406133264391208962",
      {
        method: "GET",
        headers: {
    ...
    if (info.data.spotify == null) {
      ...
      song =
        info.data.spotify.song +
        " - " +
        info.data.spotify.artist.replace(";", ",");
    }
    return song; // Log the result here
  } catch (error) {
    console.error("Error:\n", error);
  }
}
const songy = await getSong();
let songstatus = "hidden";

if (songy == "Not listening") {
  songstatus = "hidden";
} else {
  songstatus = "block";
}
...
<template>
  <footer
    class="p-5 flex justify-between text-white absolute z-20 w-screen bottom-0"
  >
    <p class="flex items-center gap-3" :class="songstatus">
      <svg
